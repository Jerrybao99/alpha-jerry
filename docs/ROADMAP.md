...
- [x] 在 `src/core/pipeline.py` 写采集主流程——读全量清单 → 低并发线程池采集 → 缓存原始响应 → 字段标准化；单股失败入 `data/fin/YYMMDD-失败.xlsx`。

  ```bash
  
  git commit -m "feat(data): 采集编排与缓存、失败隔离"
  ```
...
- **涉及文件**：`scripts/smoke_collect.py`、`src/reports/xlsx_writer.py`、`integrated_tests/test_smoke_collect.py`
- **关键命令**：
  ```bash
...
- **断点提交**：
  ...
  git commit -m "test(data): 随机5股冒烟测试与xlsx落地"
  ```
...
- **涉及文件**：`src/core/pipeline.py`（扩展）、`src/reports/xlsx_writer.py`（扩展）、`integrated_tests/test_score_rate.py`
...
- **断点提交**：
  ...
  git commit -m "feat(rating): 评分评级串联并落地xlsx"
  ```
